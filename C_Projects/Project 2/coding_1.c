...
int *solution(int L, char *S, int *res) {
    ...
    // This while loop allows to go through the string 
    while(i < L){
        if(i == 0){
            res[i] = tmp;
            continue;
        }
        // if the index of I is greater than the index of i - 1
        // add 1 to tmp and set the index of res to be tmp
        if(S[i] > S[i - 1]){
            tmp +=1;  
            res[i] = tmp;
            
        // if not, then reset tmp and set the index of res[i] = 1
        // thus reseting the count
        }else{
            tmp = 1;
            res[i] = 1;
